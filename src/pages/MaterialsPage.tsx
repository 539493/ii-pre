...
function toolbarInputClassName() {
  return "h-10 w-full rounded-[18px] border border-[#e7e1d8] bg-white px-3.5 text-[14px] text-[#223761] outline-none transition placeholder:text-[#8b99b4] focus:border-[#cedcff] focus:ring-4 focus:ring-[#2563eb]/8";
}

function FilePreviewCard({ item }: { item?: KnowledgeItem }) {
  return (
    <DashboardPanel className="min-h-[124px] p-4">
      <MiniDocumentIllustration />
      {item ? (
        <>
          <p className="mt-4 line-clamp-1 text-[13px] font-medium text-[#223761]">{item.title}</p>
          <p className="mt-1.5 line-clamp-2 text-[11px] leading-5 text-[#8a97b2]">{item.content}</p>
        </>
      ) : (
        <>
          <div className="mt-4 h-2 w-20 rounded-full bg-[#eef2f8]" />
          <div className="mt-1.5 h-2 w-14 rounded-full bg-[#f2f5fa]" />
        </>
      )}
    </DashboardPanel>
  );
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <label className="relative max-w-[286px] flex-1">
            ...
            <input
              ...
              className={cn(toolbarInputClassName(), "pl-10")}
            />
          </label>
          ...
          <select
            ...
            className="h-10 rounded-[18px] border border-[#e7e1d8] bg-white px-3.5 text-[14px] text-[#415276] outline-none transition focus:border-[#cedcff] focus:ring-4 focus:ring-[#2563eb]/8"
          >
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-[18px] border border-[#bcd0ff] bg-white px-4 text-[14px] font-medium text-[#2563eb] transition hover:border-[#9cb8ff] hover:bg-[#f6f9ff]"
          >
    ...
    >
      {filteredItems.length === 0 ? (
        ...
        <section className="grid grid-cols-1 gap-3.5 lg:grid-cols-2">
          {filteredItems.map((item) => (
            <DashboardPanel key={item.id} className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <MiniDocumentIllustration />
                  <div className="min-w-0">
                    <h3 className="line-clamp-1 text-[16px] font-semibold tracking-[-0.03em] text-[#162d58]">
                      {item.title}
                    </h3>
              ...
              <p className="mt-4 line-clamp-5 text-[13px] leading-6 text-[#7282a0]">
                {item.content}
              </p>
              ...
              <div className="mt-4 flex items-center justify-between gap-3">
                ...
                <span className="text-[12px] font-medium text-[#8a97b2]">В библиотеке знаний</span>
              </div>
            </DashboardPanel>
          ))}
        </section>
      ...
      <section className="space-y-3.5">
        <div className="flex items-center justify-between gap-3">
          ...
          <button type="button" className="text-[13px] text-[#5f7fc0] transition hover:text-[#2563eb]">
            Смотреть все
          </button>
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#10244d]/25 px-4 backdrop-blur-md">
          <DashboardPanel className="w-full max-w-lg p-4 sm:p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              <textarea
                ...
                rows={7}
                className="w-full rounded-[20px] border border-[#e7e1d8] bg-white px-4 py-3 text-[14px] leading-6 text-[#223761] outline-none transition placeholder:text-[#8b99b4] focus:border-[#cedcff] focus:ring-4 focus:ring-[#2563eb]/8"
              />

              {error && (
                <p className="text-[12px] text-[#d55656]">{error}</p>
              )}
            </div>
            ...
            <div className="mt-4 flex flex-col gap-2.5 sm:flex-row">
              ...
              <button
                ...
                disabled={saving}
                className="flex-1 rounded-[18px] bg-[#175cdf] px-4 py-2 text-[13px] font-semibold text-white shadow-[0_18px_35px_rgba(23,92,223,0.22)] transition hover:bg-[#144fc1] disabled:cursor-not-allowed disabled:bg-[#98a2b3] disabled:shadow-none"
              >
